...
Form::Form(std::string name, const unsigned int g1, const unsigned int g2) : _name(name), _signGrade(g1), _executeGrade(g2), _signed(1)
{
	if (g1 < 1 || g2 < 1)
...
Form::Form(void) : _name("no name"), _signGrade(0), _executeGrade(0), _signed(1)
{}
...
void		    Form::setName(const std::string str)
{
	const_cast<std::string&> (_name)= str;
}

void		    Form::setsignGrade(const unsigned int x)
{
	(const_cast<unsigned int&> (_signGrade) ) = x;
}

void		    Form::setexecuteGrade(const unsigned int x)
{
	const_cast<unsigned int&> (_executeGrade)= x;
}
